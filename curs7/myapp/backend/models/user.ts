...
export const create = (user: User, callback: Function) => {
  const queryString =
    "INSERT INTO jsusers (nume, prenume, email, poza, datanastere, telefon , cnp) VALUES (?, ?, ?, ?, ?,?)";
    console.log(user);
  db.query(
    queryString,
...
export const update = (user: User, callback: Function) => {
  ...
  db.query(queryString, [user.nume, user.prenume, user.telefon,user.email,user.cnp,user.id], (err, result) => {
    if (err) {
      callback(err);
    }
    callback(null);
