...
const create = (user, callback) => {
    const queryString = "INSERT INTO jsusers (nume, prenume, email, poza, datanastere, telefon , cnp) VALUES (?, ?, ?, ?, ?,?)";
    console.log(user);
...
const update = (user, callback) => {
    const queryString = `UPDATE jsusers SET nume=?, prenume=? WHERE id=? telefon=? , email=? ,cnp=?,WHERE id=?`;
    db_1.db.query(queryString, [user.nume, user.prenume, user.telefon, user.email, user.cnp, user.id], (err, result) => {
        if (err) {
            callback(err);
        }
        callback(null);
